...
| **BYOK fallback** | Protocol-specific API proxy at `/api/proxy/{anthropic,openai,azure,google,ollama,senseaudio,venice}/stream` — paste `baseUrl` + `apiKey` + `model`, choose Anthropic / OpenAI / Azure OpenAI / Google Gemini / Ollama Cloud / SenseAudio / **Venice**, and the daemon normalizes SSE back to the same chat stream. **SenseAudio and Venice chat additionally expose `generate_image` / `generate_video` / `generate_speech` tools** so the model can write rendered artifacts straight into the active project's folder. Venice deserves the dedicated mention — it's the only commercial provider whose positioning matches OD's local-first ethos: **zero data retention** (prompts and outputs never stored or logged by Venice), **optional TEE + E2EE** for the most sensitive client work (`tee-*` / `e2ee-*` models run inside Intel TDX / NVIDIA Confidential Computing enclaves with cryptographic attestation), **anonymous x402 wallet auth** if you'd rather not register an account, and **~230 multimodal models behind one key** — chat (GPT-5.5 family, Claude Opus 4.7 + Sonnet 4.6, Qwen3-Coder 480B, Llama 3.1 405B, DeepSeek V4-Pro / R1, Grok-4, Mistral, GLM-5, `venice-uncensored`) + image (`gpt-image-2`, `nano-banana-pro`, FLUX 2, Seedream v5, Recraft, Grok Imagine, `venice-sd35`) + video (Wan 2.6, Seedance 2.0 with native audio, Grok Imagine, Topaz upscale) + OpenAI-compatible TTS. Internal-IP/SSRF blocked at the daemon edge. |
