...
| **BYOK fallback** | Protocol-specific API proxy at `/api/proxy/{anthropic,openai,azure,google,ollama,senseaudio,venice}/stream` — paste `baseUrl` + `apiKey` + `model`, choose Anthropic / OpenAI / Azure OpenAI / Google Gemini / Ollama Cloud / SenseAudio / **Venice**, and the daemon normalizes SSE back to the same chat stream. **SenseAudio and Venice chat additionally expose `generate_image` / `generate_video` / `generate_speech` tools** so the model can write rendered artifacts straight into the active project's folder. With Venice a single API key unlocks chat (GPT, Claude, Qwen, Grok, DeepSeek, Llama …) + image (`gpt-image-2`, `nano-banana-pro`, `flux-2-*`, `venice-sd35`, `qwen-image-*`, `seedream-*`) + video (Wan 2.6, Seedance 2.0, Grok Imagine, Topaz upscale) + speech (OpenAI-compatible TTS) behind one provider slot. Internal-IP/SSRF blocked at the daemon edge. |
