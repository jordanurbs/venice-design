...
| **BYOK fallback** | Protocol-specific API proxy at `/api/proxy/{anthropic,openai,azure,google,ollama,senseaudio,venice}/stream` — paste `baseUrl` + `apiKey` + `model`, choose Anthropic / OpenAI / Azure OpenAI / Google Gemini / Ollama Cloud / SenseAudio / **Venice**, and the daemon normalizes SSE back to the same chat stream. **SenseAudio and Venice chat additionally expose `generate_image` / `generate_video` / `generate_speech` tools** so the model can write rendered artifacts straight into the active project's folder. Venice is the only commercial provider whose positioning matches OD's local-first ethos: **zero data retention** (prompts and outputs never stored or logged by Venice), **anonymous x402 wallet auth** if you'd rather not register an account, and **~230 multimodal models behind one key** — chat (GPT-5.5 family, Claude Opus 4.7 + Sonnet 4.6, Qwen3-Coder 480B, Llama 3.1 405B, DeepSeek V4-Pro / R1, Grok-4, Mistral, GLM-5, `venice-uncensored`) + image (`gpt-image-2`, `nano-banana-pro`, FLUX 2, Seedream v5, Recraft, Grok Imagine, `venice-sd35`) + video (Wan 2.6, Seedance 2.0 with native audio, Grok Imagine, Topaz upscale) + OpenAI-compatible TTS. Venice also runs `tee-*` models inside Intel TDX / NVIDIA Confidential Computing enclaves with cryptographic attestation — reachable today by typing the slug into the model field, though the attestation-verification + ephemeral-key E2EE handshake isn't yet wired into the OD UI. Internal-IP/SSRF blocked at the daemon edge. |
...
- **Multi-provider BYOK proxy.** `POST /api/proxy/{anthropic,openai,azure,google,ollama,senseaudio,venice}/stream` takes `{ baseUrl, apiKey, model, messages }`, builds the provider-specific upstream request, normalizes SSE chunks into `delta/end/error`, and allows loopback local LLM providers while rejecting non-loopback private, link-local, CGNAT, multicast, reserved, and redirect targets to head off SSRF. OpenAI-compatible covers OpenAI, Azure AI Foundry `/openai/v1`, DeepSeek, Groq, MiMo, OpenRouter, Ollama, LM Studio, and self-hosted vLLM; Azure OpenAI adds deployment URL + `api-version`; Google uses Gemini `:streamGenerateContent`. **Venice** speaks OpenAI's `/chat/completions` shape but also exposes `generate_image` / `generate_video` / `generate_speech` tools backed by Venice's own `POST /image/generate`, `POST /video/queue` + `POST /video/retrieve`, and `POST /audio/speech` — a single API key drives chat, image, video, and TTS through one proxy.
...
- [x] Multi-provider BYOK proxy (`/api/proxy/{anthropic,openai,azure,google,ollama,senseaudio,venice}/stream`) with SSRF guard
